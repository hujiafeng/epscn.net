...
<div class="pad_10">
  ...
  <?php if($bz == 1): ?><form name="myform" action="<?php echo U('Gongdan/inform');?>" method="post" id="form">
  <?php else: ?>
  <form name="myform" action="<?php echo U('Gongdan/bukuan');?>" method="post" id="myform"><?php endif; ?>
  <input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
   <table width="1000px" class="table_form contentWrap">
		...
		<tr>
            ...
            <td>
			￥
			<?php if($bk == 1): ?><input type="text" name="info[reward]" style="width:100px;" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9.]/g,'');}).call(this)">
			<?php else: ?>
				<?php echo ($info["reward"]); ?>
				<?php if($info["reward_m"] == '1' ): ?><font color="red">(账外收入)</font><?php endif; endif; ?>
			...
			<?php if($info["reward_m_f"] == '1' ): ?><font color="red">(账外支出)</font><?php endif; ?>
			</td>
       	</tr>
		<tr>
        ...
        <?php if($bz == 1): ?><tr>
			<td colspan="4" class="need_td">备注内容：<input type="text" name="info[content]" class="input-text" maxlength="40" style="width:500px;" id="content" required placeholder="10~20个字">
            <input name="dosubmit" type="submit"  value="提交" class="btn  btn-primary btn_submit" id="submit" style=" margin-top:-10px;">
            </td>
		</tr><?php endif; ?>
        </tbody>
      </table>
