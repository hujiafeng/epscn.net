...
<style type="text/css">
    ...
    <ul class="nav nav-tabs">
		<li class="active"><a href='#' ><?php echo ($info["name"]); ?></a></li> 
     </ul>
<div class="pad_10">
  <div class="common-form">
    <form name="myform" action="<?php echo U('Fwuser/task');?>" method="post" id="myform">
	<input type="hidden" name="uid" value="<?php echo ($info["uid"]); ?>" />
	<input type="hidden" name="name" value="<?php echo ($info["name"]); ?>" />
<table width="100%" class="table_form contentWrap">

        <tbody>
		<?php if(is_array($task_n)): foreach($task_n as $k=>$v): ?><tr>
            <td width="80"><?php echo ($v["title"]); ?></td>
            <td><input type="checkbox" value="<?php echo ($v["title"]); ?>" name="task[]" <?php echo ($v["checked"]); ?>></td>
